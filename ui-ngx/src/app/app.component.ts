...
export class AppComponent implements OnInit {
  ...
  constructor(private store: Store<AppState>,
    ...
    this.matIconRegistry.addSvgIconSetInNamespace('mdi', this.domSanitizer.bypassSecurityTrustResourceUrl('./assets/mdi.svg'));

    this.matIconRegistry.addSvgIconLiteral('alpha-a-circle-outline', this.domSanitizer.bypassSecurityTrustHtml(
    ...
    this.matIconRegistry.addSvgIconLiteral('alpha-e-circle-outline', this.domSanitizer.bypassSecurityTrustHtml(
  ...
  setupAuth() {
    ...
    ).pipe(map(results => ({isAuthenticated: results[0], isUserLoaded: results[1]})),
      distinctUntilChanged(),
      filter((data) => data.isUserLoaded ),
      skip(1),
    ).subscribe((data) => {this.authService.gotoDefaultPlace(data.isAuthenticated);});
